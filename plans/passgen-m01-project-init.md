...
- [x] Step 1: `go mod init github.com/youyo/passgen` を実行
  - Go の最新安定版（1.24 系）を使用
  - module パス: `github.com/youyo/passgen`

- [x] Step 2: `main.go` を作成
  ```go
  package main
...
- [x] Step 3: `.gitignore` を作成
  - Go バイナリ（`/passgen`）
  - OS ファイル（`.DS_Store`）
  - IDE ファイル（`.idea/`, `.vscode/`）
  - `dist/`（goreleaser 出力）

- [x] Step 4: `Makefile` を作成
...
- [x] `go build ./...` が exit code 0
- [x] `go test ./...` が exit code 0
- [x] `go vet ./...` が exit code 0
- [x] `make build` で `passgen` バイナリが生成される
- [x] `./passgen` を実行して "passgen" が出力される
- [x] `.gitignore` にバイナリと OS ファイルが含まれている
